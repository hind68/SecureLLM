...
describe('ChatMessage', () => {
  ...
  it('styles text DLP tabs and code highlights as compact segmented controls', () => {
    ...
    expect(messagesCss).toMatch(/\.dlp-code-view mark\s*\{[^}]*background:\s*rgba\(45,\s*212,\s*191,\s*0\.1\);/is)
    expect(messagesCss).toMatch(/\.dlp-code-view mark\s*\{[^}]*border:\s*1px solid rgba\(45,\s*212,\s*191,\s*0\.24\);/is)
    expect(messagesCss).toMatch(/\.dlp-code-view mark\s*\{[^}]*color:\s*#7DD3FC;/is)
    ...
    expect(cssRule(messagesCss, '.dlp-detection-badge')).toMatch(/font-weight:\s*650;/i)
  ...
  it('calculates threat lines from extracted text offsets and wires cards to highlights', () => {
    ...
    expect(html).toContain('document-threat-list shrink-0 is-collapsed')
  ...
  it('keeps the inspector scroll inside the content area and the composer centered in chat space', () => {
    ...
    expect(cssRule(panelsCss, '.document-text-size-trigger')).toMatch(/font-size:\s*18px;/)
    ...
    expect(cssRule(panelsCss, '.document-dlp-mark')).toMatch(/background-color:\s*rgba\(100,\s*116,\s*139,\s*0\.08\);/)
    expect(cssRule(panelsCss, '.document-dlp-mark')).toMatch(/color:\s*#334155;/)
    ...
    expect(cssRule(composerCss, '.composer-attachments')).toMatch(/padding:\s*4px 50px 6px 4px;/)
  ...
  it('opens blocked file messages from the clickable attachment card', () => {
    ...
    expect(cssRule(messagesCss, '.dlp-file-panel .file-message-card.dlp-file-card')).toMatch(/border:\s*1px solid rgba\(248,\s*113,\s*113,\s*0\.18\);/)
    expect(cssRule(messagesCss, '.dlp-file-panel .file-message-card.dlp-file-card')).toMatch(/rgba\(255,\s*255,\s*255,\s*0\.56\)/)
    ...
    expect(cssRule(messagesCss, '.dlp-file-panel .file-message-card.dlp-file-card.is-clickable:hover')).toMatch(/rgba\(255,\s*255,\s*255,\s*0\.78\)/)
    ...
    expect(cssRule(messagesCss, '.dlp-file-panel .file-message-card.dlp-file-card.is-clickable:hover')).toMatch(/border-color:\s*rgba\(248,\s*113,\s*113,\s*0\.32\);/)
